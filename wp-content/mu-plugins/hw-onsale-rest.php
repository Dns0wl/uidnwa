...
function hw_format_products_for_rest(array $ids, array $context = [])
{
    ...
    foreach ($ids as $id) {
        ...
        $is_first = ($page <= 1 && $offset === 0 && empty($products));
        if ($image) {
            $image['loading'] = $is_first ? 'eager' : 'lazy';
            $image['fetchpriority'] = $is_first ? 'high' : 'auto';
        }
