...
if (! function_exists('hw_get_optimized_product_image')) {
    function hw_get_optimized_product_image($image_id)
    {
        ...
        $aspect_ratio = $base_width > 0 ? $base_height / $base_width : 1.25;
        $max_width = $full ? max($base_width, (int) $full[1]) : $base_width;

        $targets = [320, 480, 640, 800, 960, 1200];
        $widths = [];
        ...
        if (empty($widths)) {
            $widths[] = min($max_width, max($targets));
        }
        ...
        $preferred_width = 640;
        $base_variant_width = end($widths);
        foreach ($widths as $variant_width) {
            if ($variant_width >= $preferred_width) {
                $base_variant_width = $variant_width;
                break;
            }
        }
        ...
        $placeholder = hw_cloudflare_format_image_url($base_url, [
            'format'   => 'webp',
            'width'    => max(16, (int) round($base_variant_width / 14)),
            'height'   => max(16, (int) round($base_variant_height / 14)),
            'fit'      => 'cover',
            'quality'  => 40,
            'blur'     => 50,
            'metadata' => 'none',
...
add_filter('woocommerce_get_product_thumbnail', function ($html, $size, $args) {
    ...
    $is_lcp = (1 === $rendered && ! is_paged());

    if ($is_lcp && function_exists('hw_perf_archive_register_lcp_image')) {
    ...
    <picture>
        ...
        <img src="<?php echo esc_url($image['src']); ?>"
            ...
            fetchpriority="<?php echo $is_lcp ? 'high' : 'auto'; ?>"
