...
(function() {
	...
	function applyGridStyles() {
		...
		if (!grid || !window.hwOnsale) return;

		const settings = window.hwOnsale.settings || {};
       ...
       function openProductModal(card) {
               ...
               if (modalElements.imageWrapper) {
                ...
                if (modalElements.feedback) {
                        modalElements.feedback.textContent = window.hwOnsale?.i18n?.loading || 'Loading...';
                }
                ...
                fetchProductOptions(productId)
                        ...
                        .catch(function() {
                                if (modalElements.feedback) {
                                        modalElements.feedback.textContent = window.hwOnsale?.i18n?.loadError || 'Unable to load options.';
                                }
                        });
        }
        ...
        function fetchProductOptions(productId) {
                ...
                if (!window.hwOnsale?.restUrl) {
                        return Promise.reject(new Error('Missing endpoint'));
                }

                const url = window.hwOnsale.restUrl.replace(/\/$/, '') + '/product/' + productId + '/options';

                return fetch(url, {
                        method: 'GET',
                        credentials: 'same-origin',
                        headers: {
                                'X-WP-Nonce': window.hwOnsale?.nonce || ''
                        }
                })
        ...
        function renderProductOptions(data) {
                ...
                if (!Array.isArray(data?.attributes) || data.attributes.length === 0) {
                        ...
                        if (!data?.simple_add_to_cart_url && modalElements.feedback && !hasInStockVariations) {
                                modalElements.feedback.textContent = window.hwOnsale?.i18n?.outOfStock || 'Out of stock';
                        }
                ...
                data.attributes.forEach(function(attribute) {
                       ...
                       placeholder.value = '';
                       let placeholderText = (attribute.placeholder || window.hwOnsale?.i18n?.chooseOption || 'Choose an option');
                ...
                function handleChange() {
                        ...
                        if (matched && matched.is_in_stock && matched.add_to_cart_url) {
                                ...
                                if (modalElements.feedback) {
                                        modalElements.feedback.textContent = window.hwOnsale?.i18n?.unavailable || 'Combination unavailable';
                                }
	...
	function initializeTracking() {
		if (!window.hwOnsale?.settings?.trackingEnabled) {
			return;
		}
	...
	function loadMore() {
		...
		const batchSize = window.hwOnsale?.settings?.batchSize || 12;
		...
		fetch(window.hwOnsale.restUrl + '/list?' + params.toString())
			.then(function(response) {
			...
			.then(function(data) {
				if (data.cards && data.cards.length > 0) {
					appendProducts(data.cards);
					
					// Restore scroll position
					window.scrollTo(0, scrollY);
					
					// Check if more products available
					if (currentOffset + batchSize >= data.found) {
						hasMore = false;
	...
	function createProductCard(product) {
		...
		template.innerHTML = renderProductCard(product).trim();
	 ...
	 */
        function renderProductCard(product) {
                const badgeThreshold = window.hwOnsale?.settings?.badgeThreshold || 0;
                const showBadge = product.discount_pct > badgeThreshold;
                const badgePosition = window.hwOnsale?.settings?.badgePosition || 'top-left';
                const badgeStyle = window.hwOnsale?.settings?.badgeStyle || 'solid';
                const categoriesJson = JSON.stringify(Array.isArray(product.categories) ? product.categories : (Array.isArray(product.materials) ? product.materials : []));
                const categoriesAttr = escapeHtml(categoriesJson || '[]');
                const sizeAttr = escapeHtml(product.size || '');

                const truncatedName = truncateWords(product.name, 4);

               let html = '<div class="hw-onsale-card" data-product-id="' + product.id + '" data-discount="' + product.discount_pct + '" data-variable="' + (product.is_variable ? '1' : '0') + '" data-categories="' + categoriesAttr + '" data-size="' + sizeAttr + '" data-product-name="' + escapeHtml(product.name || '') + '" data-product-link="' + escapeHtml(product.permalink || '') + '">';

		if (showBadge) {
			html += '<div class="hw-onsale-badge hw-onsale-badge--' + badgePosition + ' hw-onsale-badge--' + badgeStyle + '" role="text" aria-label="' + product.discount_pct + '% off">-' + product.discount_pct + '%</div>';
		}

                html += renderSlider(product.images, product.name, product.permalink);

                html += '<div class="hw-onsale-card__content">';
                html += '<h3 class="hw-onsale-card__title"><a href="' + escapeHtml(product.permalink) + '" data-modal-trigger="title">' + escapeHtml(truncatedName || product.name || '') + '</a></h3>';
                const quickLabel = 'View quick actions for ' + (product.name || 'this product');
                html += '<div class="hw-pricewrap" role="button" tabindex="0" data-modal-trigger="price" aria-label="' + escapeHtml(quickLabel) + '">';
                html += '<div class="hw-onsale-card__price">' + product.price_html + '</div>';
                html += '</div></div></div>';
	 ...
	 * Render slider HTML
	 */
        function renderSlider(images, alt, permalink) {
                let html = '<div class="hw-onsale-slider" role="region" aria-label="Product images" data-modal-trigger="slider">';
                ...
                if (Array.isArray(images) && images.length > 1) {
                        const dotCount = Math.min(images.length, 3);
                        html += '<div class="hw-onsale-slider__dots" role="tablist" aria-label="Image navigation">';
                        for (let i = 0; i < dotCount; i++) {
                                const isActive = i === 0 ? ' is-active' : '';
                                let targetSlide;
                                if (images.length <= dotCount) {
                                        targetSlide = i;
                                } else if (i === 0) {
                                        targetSlide = 0;
                                } else if (i === dotCount - 1) {
                                        targetSlide = images.length - 1;
                                } else {
                                        targetSlide = Math.round((images.length - 1) / 2);
                                }
                                const labelNumber = targetSlide + 1;
                                html += '<button type="button" class="hw-onsale-slider__dot' + isActive + '" role="tab" aria-selected="' + (i === 0 ? 'true' : 'false') + '" data-dot-index="' + i + '" data-target-slide="' + targetSlide + '" aria-label="Image ' + labelNumber + '"><span class="hw-sr-only">Go to image ' + labelNumber + '</span></button>';
                        }
                        html += '</div>';
                }
	...
	function trackEvent(event, data) {
		...
		const payload = {
			...
			_wpnonce: window.hwOnsale?.nonce || ''
		};

		// Use sendBeacon for reliability
		if (navigator.sendBeacon) {
			navigator.sendBeacon(
				window.hwOnsale.restUrl + '/event',
